...
% ===== 本文幅：表・見出しラインをヘッダより少し狭く統一 =====
\newlength{\SpecBodyWidth}
\setlength{\SpecBodyWidth}{160mm}
...
\hline
\Lab{図面番号} & \DrawingNo & \Lab{図面 Rev.} & 記入 \\
\hline
\Lab{仕様書番号} & \SpecNo & \Lab{仕様書 Rev.} & \SpecRev \\
\hline
\Lab{希望数量} & \Quantity & \Lab{希望納期} & \DueDate \\
\hline
\Lab{作成部門 / 作成者} & 記入 & \Lab{取引先 / 製作者} & 記入 \\
\hline
\Lab{作成日} & \SpecDate & \Lab{承認者} & 記入 \\
\hline
\end{SpecTable}
...
\begin{enumerate}
  ...
  \item その他参考資料。
\end{enumerate}
\Note{不一致、疑義、製作上の制約を発見した場合は、製作前に発注者へ確認すること。}

\BlockTitle{2.4 支給資料}
\begin{SpecTable}{|p{46mm}|p{18mm}|p{34mm}|p{48mm}|}
\hline
\Head{資料名} & \Head{Rev.} & \Head{形式} & \Head{備考} \\
\hline
製作図 &  & PDF / DXF &  \\
\hline
