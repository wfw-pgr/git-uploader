...
# ========================================================= #
# ===  convert__femtet2phits                            === #
# ========================================================= #

def convert__femtet2phits( inpFile=None, outFile=None, density=None, densityFile=None, \
                           index_plus_1=True ):
    ...
    if ( outFile is None ): outFile = inpFile.replace( ".bdf", "_phits.bdf" )
    if ( density is None ):
        if ( densityFile is None ):
            print( "[convert__femtet2phits.py] density == ??? , densityFile == ??? [ERROR]" )
            sys.exit()
        else:
            with open( densityFile, "r" ) as f:
                density = json5.load( f )


    # ------------------------------------------------- #
    # --- [2] load mesh elements & nodes            --- #
    # ------------------------------------------------- #
    print( "[convert__femtet2phits.py] loading Nastran File :: {0}".format( inpFile ), end=""  )
    rmesh     = meshio.read( inpFile )
    points    = rmesh.points
    cells     = rmesh.cells_dict["tetra"]
    if ( "nastran:ref" in rmesh.cell_data_dict ):
        physNums  = np.array( rmesh.cell_data_dict["nastran:ref"]["tetra"] )
    else:
        sys.exit( " no nastran:ref " )
    ...
    f = open( outFile, "w" )
    f.write( "$ Generated by nkMeshRoutines.convert__femtet2phits by N.K." + "\n" )
    f.write( "BEGIN BULK" + "\n" )
    ...
    physNums_set    = list( set( physNums ) )
    ...
    for key in keys:
        if ( key in density ):
            ...
            print( "[convert__femtet2phits.py] Cannot Find {} in density dict.... [ERROR]".format( key) )
            sys.exit()
...
if ( __name__=="__main__" ):
    densityFile = "density.json"
    inpFile     = "rewrite.bdf"
    convert__femtet2phits( inpFile=inpFile, densityFile=densityFile )
