...
def prepare( ctx ):
    ...
    t2i.translate__track2impactx( paramsFile   = paramsFile, \
                                  trackFile    = trackFile, \
...
def post( ctx, paramsFile="dat/parameters.json", \
          refp=False, stat=True, trajectory=False, vtk=False, postProcessed=True, \
          ext=".0" ):
    ...
    if ( postProcessed ):
        ...
        ptk.plot__postProcessed( paramsFile=paramsFile, plot_conf=plot_conf )
    return()
...
# ===  all = clean + run + post                         === #
# ========================================================= #
@invoke.task(pre=[clean, prepare, run, post])
def all(ctx):
