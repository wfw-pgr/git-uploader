...
# ========================================================= #
def calculate__twissFromTrackv38( Win=5.0e6, freqb=36.5e6, A=2.0, mass_amu=2.014, full2rms=1.0/8.0,
                                  epsnx=20.0, alfax=0.173982, betax=535.875,
                                  epsny=20.0, alfay=0.115839, betay=113.997,
                                  epsnz=5.5,  alfaz=0.0,      betaz=10.0,  ):
    cv    = 299792458.0
    amu   = 931.4941024e6
    cm    = 1.0e-2
    cm2mm = 10.0
    ...
    # args = {
    #     "Win"   : 20.0e6 , "A"     : 2.0    , "freqb" : 36.5e6 , "mass_amu" : 2.014,
    #     "epsnx" : 15     , "alfax" : 0      , "betax" : 800    ,
    #     "epsny" : 15     , "alfay" : 0      , "betay" : 400    , 
    #     "epsnz" : 1.20   , "alfaz" : 0      , "betaz" : 20     ,
    # }
    # calculate__twissFromTrackv38( **args )

    # ------------------------------------------------- #
    # --- [1] calculation                           --- #
    # ------------------------------------------------- #
    # -- relativistic gamma, beta                   --  #
    gamma = 1.0 + Win * A / ( mass_amu * amu )
    beta  = np.sqrt( 1.0 - 1.0 / gamma**2 )

    # -- TRACKv38: Normalized full [pi cm mrad] -> geometric rms [mm mrad]  -- #
    emitx = epsnx / ( beta * gamma ) * full2rms * cm2mm
    emity = epsny / ( beta * gamma ) * full2rms * cm2mm
    ...
    betat = abs( kt / kpt ) * betaz
    emitt = abs( kt * kpt ) * ( epsnz*full2rms ) * 1.0e6
