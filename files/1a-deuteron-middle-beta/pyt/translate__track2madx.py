...
def translate__track2madx( paramsFile="dat/parameters.json" ):
    ...
    # ------------------------------------------------- #
    seq    = []
    counts = { "at":0.0, "N":0, "Nqm":0, "Nrf":0, "Ndr":0, "Nps":0 }
    for ik,line in enumerate(lines):
        words = ( line.split() )
        if ( not( len( words ) == 0 ) ):
            counts["N"] += 1   # skip at phShifter 
    ...
    phase_f   = "  {0}: matrix, L=0.0, kick5={1:.8};\n"
    drift_f   = "  {0}: drift, L={1:.8}, kick5={2:.8}, at={3:.8};\n"
...
def convert_to_rfgap( words, counts, params, Ek, E0, phase=None ):
    ...
    if ( params["mode.phase"].lower() == "adjust" ):
        if ( phase is None ): sys.exit( "[convert_to_rfgap] phase is None." )
        tag_ps1 = "ps{}".format( counts["Nps"]+1 )
        tag_ps2 = "ps{}".format( counts["Nps"]+2 )
        shiftT  = phase[ phase["NAME"]==tag]["SHIFT"]
        print( shiftT )
        shift1  = (-1.0) * shiftT
        shift2  = (+1.0) * shiftT
