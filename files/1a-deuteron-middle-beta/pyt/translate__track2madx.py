...
def translate__track2madx( paramsFile="dat/parameters.json" ):
    ...
    if ( params["mode.phase"].lower() in [ "adjust", "direct" ] ):
        if ( os.path.exists( params["file.phase"] ) ):
            with open( params["file.phase"] ) as f:
                phase = pd.read_csv( f )
        else:
            phase = None
            print( "[translate__track2madx.py] phase file is not given... " )
            
    elif ( params["mode.phase"].lower() in [ "none" ] ):
        phase = None
    else:
        print( "unknown params['mode.phase'] == {}".format( params["mode.phase"] ) )
        sys.exit()
    ...
    # ------------------------------------------------- #
    seq    = []
    counts = { "at":0.0, "N":0, "Nen":0, "Nqm":0, "Nrf":0, "Ndr":0, "Nps":0 }
    for ik,line in enumerate(lines):
        words = ( line.split() )
        if ( not( len( words ) == 0 ) ):
            counts["N"]   += 1
            counts["Nen"] += 1
            Ek = energy["Energy[MeV/u]"][ counts["Nen"] ] * params["Nu"]      #  -- [MeV]
            ...
            if   ( words[1].lower()  == "quad" ):
                ...
                seq += convert_to_rfgap( words, counts, params, Ek, E0, phase=phase )
            
            else:
                sys.exit( "[ERROR] undefined keyword :: {} ".format( words[1] ) )

    L_tot     = counts["at"]
    Nelements = counts["Nen"]
    ...
    phase_f   = "  {0}: matrix, L=0.0, kick5={1:.8}, at={2:.8};\n"
    ...
    for ik,elem in enumerate(seq):
        ...
        elif ( elem["type"].lower() == "phshifter"  ):
            keys      = [ "tag", "kick5", "at" ]
            contents += phase_f.format( *( [ elem[key] for key in keys ] ) )
    ...
    if ( params["ptc.sw"] ):
        ...
        cmd = "observe"
    for ik,elem in enumerate(seq):
        if ( ( elem["type"] in [ "quadrupole", "rfcavity", "rfgap" ] ) ):
            contents += "{0}, place={1};\n".format( cmd, elem["tag"] )
...
# ===  convert_to_rfgap                                 === #
# ========================================================= #
def convert_to_rfgap( words, counts, params, Ek, E0, phase=None ):
    ...
    if   ( params["translate.RFmode"] == "rfcavity" ):
        ...
        lag      = ( float(words[3]) + shift ) / 360.0 * params["translate.lagSign"]
        harmon   =     int(words[4])
    ...
    if ( params["mode.phase"].lower() == "adjust" ):
        if ( phase is None ):
            shiftT = 0.0
        else:
